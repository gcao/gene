import parseopt, strutils, os, tables
import ../types
import ../parser
import ./base
    ...
    of cmdShortOption, cmdLongOption:
      if p.val == "":
          ...
          if p.key in ["e", "eval", "f", "format"]:
            ...
            if p.kind == cmdArgument:
              ...
              of "e", "eval":
                result.code = p.key
              of "f", "format":
                if p.key in ["pretty", "compact", "sexp"]:
                  result.format = p.key
                else:
                  stderr.writeLine("Error: Invalid format '" & p.key & "'. Must be: pretty, compact, or sexp")
                  quit(1)
            else:
        ...
        of "e", "eval":
          result.code = p.val
        of "f", "format":
          if p.val in ["pretty", "compact", "sexp"]:
            result.format = p.val
          else:
            stderr.writeLine("Error: Invalid format '" & p.val & "'. Must be: pretty, compact, or sexp")
            quit(1)
        else:

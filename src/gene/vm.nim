...
proc exec*(self: VirtualMachine): Value =
  ...
  while true:
    ...
    case inst.kind:
      ...
      of IkCallDirect1:
        # Direct call with 1 arg (specialized for performance)
        {.push checks: off}
        let arg_value = self.frame.pop()  # Pop the argument
        let fn_value = self.frame.pop()   # Pop the function
        ...
        case fn_value.kind:
          of VkFunction:
            ...
            # Set up scope
            if f.matcher.is_empty():
              new_frame.scope = f.parent_scope
            else:
              new_frame.scope = new_scope(f.scope_tracker, f.parent_scope)
            
            # Optimize for simple single-parameter case
            if not f.matcher.is_empty() and f.matcher.children.len == 1:
              # Direct assignment for single parameter - avoid Gene creation
              let param = f.matcher.children[0]
              if param.kind == MatchData and not param.is_splat and param.children.len == 0:
                # Simple parameter binding - most common case
                # Find the index for this parameter
                if f.scope_tracker.mappings.has_key(param.name_key):
                  let idx = f.scope_tracker.mappings[param.name_key]
                  # Ensure scope has enough space
                  while new_frame.scope.members.len <= idx:
                    new_frame.scope.members.add(NIL)
                  new_frame.scope.members[idx] = arg_value
                # Still need to set args for compatibility
                var args = new_gene(NIL)
                args.children.add(arg_value)
                new_frame.args = args.to_gene_value()
              else:
                # Complex matcher - use normal processing
                var args = new_gene(NIL)
                args.children.add(arg_value)
                new_frame.args = args.to_gene_value()
                process_args(f.matcher, new_frame.args, new_frame.scope)
            elif not f.matcher.is_empty():
              # Multiple parameters or complex matcher
              var args = new_gene(NIL)
              args.children.add(arg_value)
              new_frame.args = args.to_gene_value()
              process_args(f.matcher, new_frame.args, new_frame.scope)
            else:
              # No matcher - just set args
              var args = new_gene(NIL)
              args.children.add(arg_value)
              new_frame.args = args.to_gene_value()
          ...
          of VkBlock:
            let b = fn_value.ref.block
            if b.body_compiled == nil:
              b.compile()
            
            # Create a new frame for the block call
            let new_frame = new_frame()
            ...
            # Set up scope - blocks use frame.scope
            if b.matcher.is_empty():
              new_frame.scope = b.frame.scope
            else:
              new_frame.scope = new_scope(b.scope_tracker, b.frame.scope)
            
            # Optimize for simple single-parameter case (same as function)
            if not b.matcher.is_empty() and b.matcher.children.len == 1:
              let param = b.matcher.children[0]
              if param.kind == MatchData and not param.is_splat and param.children.len == 0:
                # Simple parameter binding
                if b.scope_tracker.mappings.has_key(param.name_key):
                  let idx = b.scope_tracker.mappings[param.name_key]
                  while new_frame.scope.members.len <= idx:
                    new_frame.scope.members.add(NIL)
                  new_frame.scope.members[idx] = arg_value
                var args = new_gene(NIL)
                ...
                args.children.add(arg_value)
                new_frame.args = args.to_gene_value()
                process_args(b.matcher, new_frame.args, new_frame.scope)
            elif not b.matcher.is_empty():
              var args = new_gene(NIL)
              args.children.add(arg_value)
              new_frame.args = args.to_gene_value()
              process_args(b.matcher, new_frame.args, new_frame.scope)
            ...
            # Fall back to regular calling mechanism for other types
            # We can't optimize these, so revert to normal processing
            # Push back in correct order for normal gene processing
            self.frame.push(fn_value)
            self.frame.push(arg_value)
            # Generate regular gene call instructions
            # This is not ideal but necessary for non-function/block types
            todo("IkCallDirect1: Non-function/block types not yet optimized")
        {.pop.}
